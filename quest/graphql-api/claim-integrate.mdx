...
![Diagram.png](/images/developer/graphql-api/campaign-struct.png)
...
![Diagram.png](/images/developer/graphql-api/campaign-config.png)
```
...
1. Sign up for Galxe Space Dashboard, please visit [Create a Space](https://app.galxe.com/createspace).
...
5. Create campaign with with “Integrate campaign claiming into third-party websites” checked under your space through the Dashboard. _(This will turn the campaign into gasless mode, right now we only support any campaign that does not require users to pay gas.)_
...
For endpoint and other info, please refer to [Overview | Galxe Docs](/quest/graphql-api/overview).

And for interface of each GraphQL query, please check docs/schema from [Playground - https://graphigo-business.prd.galaxy.eco/query](https://graphigo-business.prd.galaxy.eco/) .

## 2.2\. Query campaign[](#22-query-campaign "Direct link to heading")

This API is used for get campaign object, a campaign is the key object that holds all info for NFT claiming process, integrate this API to setup your own NFT claim page. An example of how a campaign can be shown on website: [New To The Galxe? Start your adventure here! by Galxe Space | Galxe](https://app.galxe.com/quest/Galxe/GCqBzUtW7w) .
...
[**Creds fields**](#2-2-3-4-creds-fields)
...
[Use API to update credential Items | Galxe Docs](https://docs.galxe.com/quest/guide/api-cred-items-update)

### 2.3.3\. check address eligiblilty Single[](#233-check-address-eligiblilty-Single "Direct link to heading")
...
### 2.3.4\. check address eligiblilty(verify button) Single[](#234-check-address-eligibliltyverify-button-single "Direct link to heading")
**NOTE:**This can only check single credential eligible.

**NOTE:**This will verify the user's data. Some cred types will have recaptcha restrictions _(if verify gets a recaptcha error)_. Verification can only be initiated from the galxe page, we will regularly adjust the types that require recaptcha.
```grapql
mutation {
...
### 2.5.3\. **Fields**[](#253-fields "Direct link to heading")
